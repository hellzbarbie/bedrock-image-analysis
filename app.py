...
# Instantiating the Bedrock client, and passing the CLI profile
...
def image_base64_encoder(image_name):
    ...
    """
    # Opening the image file that ws uploaded by the user
    ...
    # Returning both the formatted file type string, along with bas encoded image
...
def analyze_image(image_name, text) -> str:
    ...
    # The system prompt is used as a default prompt, and is always passed into the model
    ...
    # text in the text block of a prompt.
    if text == "":
    ...
    # Invoking Claude3, passing the prompt
    response = bedrock.invoke_model(body=json_prompt,
                                    modelId="anthropic.claude-3-sonnet-20240229-v1:0",
                                    accept="application/json",
                                    contentType="application/json"
                                    )
    # Getting the response from Clause3 and parsing it to return to the end user
